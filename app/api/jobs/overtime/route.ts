/**
 * Background Job: Process Overtime Request
 * 
 * This endpoint handles the heavy processing for overtime requests:
 * - Posts main message to overtime channel
 * - Saves to Google Sheets
 * - Sends DM messages to approvers
 * 
 * Called asynchronously from the Slack interactions handler to avoid timeout.
 * Secured by JOBS_SECRET Bearer token.
 */
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!process.env.JOBS_SECRET || auth !== expectedAuth) {
    ...
    if (!requesterId || !projectName || assignedByUserId === undefined) {
    ...
    if (!overtimeChannelId) {
      console.error('SLACK_OVERTIME_CHANNEL_ID is not set')
      return NextResponse.json(
        { error: 'Overtime channel not configured' },
        { status: 500 }
      )
    }
    ...
    console.error('Error processing overtime request job:', error)
    return NextResponse.json(
